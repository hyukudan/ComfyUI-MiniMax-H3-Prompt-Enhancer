...
**Storyboard** is the production editor: an always-visible Shot strip and a task-ordered Content flow of **Shot setup → Action & dialogue → Shot references → Direction**. One compact selected-Shot header carries Generation, duration, scope and Content/Stage/Camera navigation. The setup itself is visual and non-duplicated: its background card selects the reusable Place and active view, while Cast and Objects expose portrait/design cards already in the Shot plus visual **Add from Library** cards for available entities. A thin **Create new for this Shot** bar is reserved for creating new reusable resources; it does not repeat the current assignments as technical chips or a second Environment selector. **Content**, **Stage** and **Camera** are modes of the selected Shot, not separate project areas. Creating a Subject, Place or Object there creates the canonical reusable entity and adds it atomically. **Dialogue & sound** sits directly beside visible Action and authors exact lines with separate Speaker, Channel (on-screen or V.O.), Delivery and Tone controls. The Files shelf opens only when requested from Shot references; it no longer precedes the creative decisions it serves. Stage and Camera remain full specialist modes and appear at the end of Content as compact Direction summaries. Every authored Action, beat and exact dialogue line joins one LLM source contract derived from the Shot plan. The read-only **LLM handoff** is collapsed by default and derives its subject and physical aliases from the same v3 graph sent to the compiler.
...
When Storyboard has Shots, the node replaces the outer **Video description** textarea with **What Prompt Studio sends**, a compact read-only summary for the selected Generation. It exposes the Shot action, cast, Environment, dialogue/reference counts and camera status without presenting a second place to write the prompt. **Edit Storyboard** opens the owning editor. If Studio has no Shots, the legacy textarea remains available for old or simple non-Studio workflows.
...
1. Open **Storyboard** and write the visible Action in the first Shot. For an older workflow, its existing Video description can seed that first Shot once.
2. Add further Shots and create reusable resources in **Library** as needed.
...
When Studio Project v3 is empty, the legacy Video description remains editable and can seed the first Storyboard Shot. Once any Shot exists, Studio becomes the sole request source and the legacy value is preserved but not sent.
...
- a visible Action for every Shot; the first Shot may copy a legacy Video description during migration, then every Shot is authored and validated independently inside Storyboard;
...
Subjects have a stable logical ID, an H3 subject index, an identity description, identity pictures, an optional default voice asset, and a base appearance state. Identity pictures appear as selectable thumbnails in the Subject itself; **+ Identity picture** uploads and attaches one without visiting Files. **+ Import voice** does the same for the reusable voice default. A waveform trim editor beside the assigned voice selects an exact range of at most 15 seconds and previews only that range. The range belongs to the logical audio File, survives v3 compilation, and is sliced from the decoded waveform before the existing numbered `ref_audio_N` output is produced; no derivative file or extra socket is created. The editor is also present on the expanded Storyboard Subject card and automatically follows its Shot voice override. Subject cards stay as compact identity/voice summaries until **Edit references** is opened. Beside the selected Shot's Action, subject chips can be dragged into the selected Shot or activated with **Add to Shot & mention**: both cast the Subject immediately so it becomes available in Dialogue **Speaker**, while the latter also inserts `<Subject N>` at the cursor. Hover or keyboard focus previews the identity image, description and default voice before insertion. The default voice is inherited whenever the subject is active; a shot-level `voice` reference is an explicit override for that Shot. Appearance states can control wardrobe, hair, makeup, accessories, carried items, damage, wetness, body condition, transformation, or another explicitly declared dimension. Their detailed forms stay collapsed until selected so the common identity workflow remains compact.

A Subject is a reusable library definition, while prompt inclusion is an explicit casting decision. New subjects are included in the currently selected generation by default. Existing subjects can be sent without creating a Shot through **Use in prompts → Always include in Generation …**; alternatively, cast them in the selected Shot under **Storyboard · Content**. Active subjects compile into machine-derived contracts and are deterministically restored in Ref2VA `subject_definitions` with their Studio name, complete stable description, numbered identity Pictures and owned voice Audio. Their first use in `detailed_description` repeats that stable identity, rather than relying on the LLM to rediscover details such as hair or wardrobe. Review warns when a valid library Subject is neither included in a Generation nor cast in a Shot, because it will not reach the LLM.
