...
**Storyboard** is the production editor: an always-visible Shot strip and a task-ordered Content flow of **Scene setup → Action & dialogue → Shot references → Direction**. A compact horizontal **Shot context** line carries the derived Generation, Cast, References, Duration and collapsed LLM handoff above this flow; it never reserves a side column, so Scene setup uses the full editor width. **Content**, **Stage** and **Camera** are modes of the selected Shot, not separate project areas. Scene setup creates and places a canonical Subject or Environment atomically, so Ana can receive a reusable identity image and voice, enter a Shot, receive Shot-only overrides, and use a selected background without leaving Prompt Studio. **Dialogue & sound** sits directly beside visible Action and authors exact lines with separate Speaker, Channel (on-screen or V.O.), Delivery and Voice color controls. The Files shelf opens only when requested from Shot references; it no longer precedes the creative decisions it serves. Stage and Camera remain full specialist modes and appear at the end of Content as compact Direction summaries. Every authored Action, beat and exact dialogue line joins one LLM source contract derived from the Shot plan. The read-only **LLM handoff** is collapsed by default and derives its subject and physical aliases from the same v3 graph sent to the compiler.
