...
def test_look_round_trip_carries_all_creative_and_cinematography_state_but_never_shots():
    ...
    capture = _function(source, "lookEnvelopeFromNode", "serializeLookEnvelope")
    apply = _function(source, "applyLookEnvelope", "setLookStatus")
...
def test_look_import_rejects_future_schema_and_sanitizes_both_payload_halves():
    ...
    assert "sanitizeCreativeTreatment(parsed.creativeTreatment)" in sanitize
    assert "sanitizeCinematography(parsed.cinematography)" in sanitize
...
def test_look_library_is_bounded_recoverable_and_browser_local():
    ...
    assert "The shot plan was left untouched" in source
