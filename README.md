...
3. In **Storyboard · Content**, create or select a Shot, cast Ana, choose the Environment/background and write its action and dialogue. Add **This Shot** image or voice overrides only when that Shot must differ from Ana's reusable defaults.
4. Use **Storyboard · Stage** for blocking and eyelines, then **Storyboard · Camera** for that same Shot's Start, Move and End direction. The opaque sticky **Editing** context keeps the selected Shot and horizontal Shot strip visible.
...
Prompt Studio opens on **Storyboard**, the native visual prompt-authoring surface where each selected Shot follows the working order **Scene setup → Action & dialogue → Shot references → Direction**. Shot context is a compact horizontal status line above Content, so derived generation, cast, reference and duration information never consumes a permanent inspector column or narrows Scene setup. **Dialogue & sound** gives each exact line independent Speaker, Channel (on-screen or V.O.), Delivery and Voice color controls; structured Voice colors compile to the same pitch, timbre and pacing direction as the main prompt palette. Files open contextually from a reference destination instead of occupying the start of the workflow, while Stage and Camera remain full specialist modes summarized at the end of Content. One authoritative **Studio Project v3** owns Files, Subjects, Environments, Generations, Shots, links and Look inside the enhancer; no second node or user-facing companion document is required. Its integrated destinations are **Storyboard, Library, Look, and Review & Generate**. A sticky **Editing** context shows the Shot name, generation, duration and **This Shot** scope while **Content**, **Stage**, and **Camera** edit that same selected Shot.

The enhancer node stays compact through a primary **Open Studio** action and destination chips. Once Storyboard contains Shots, the old outer **Video description** editor is replaced by a read-only **What Prompt Studio sends** summary: each row shows the selected Generation's action plus cast, Environment, dialogue, Shot references and camera status. **Edit Storyboard** opens the only authoring surface. The legacy description remains editable only while Studio has no Shots, so older workflows still load without creating two competing prompt sources. Each Studio destination opens a wide, resizable viewport-level drawer that does not scale with the ComfyUI canvas. It defaults to 720 px on ordinary desktops, 820 px on wide displays, and 920 px on 4K/high-resolution displays; it is bounded between 420 px and `min(1100px, 60vw)`, becomes full-width below 700 px, and stacks master/detail editors on narrow content. Every deliberate editor change is saved immediately into the authoritative v3 workflow data—there is no separate Save button. The **Guided / Advanced** selector lives inside **Look**. Runtime v2 inputs remain hidden compatibility projections compiled from v3 for the established prompt pipeline; they are not separate Studio documents.
...
For reference workflows, **Studio Project v3 is the source of truth**. Storyboard presents a selectable Shot strip, central stage, cast/background/action controls, visual reference tray, semantic destinations and contextual inspector. Create Ana in **Library · Subjects**, see and select her identity pictures as thumbnails, or upload a new identity picture and default voice directly from her card. The integrated voice editor shows a waveform, start/end controls and a bounded preview; its selected range (up to 15 seconds) is the physical audio emitted through the assigned `ref_audio_N` output. The same editor appears on the expanded Subject card in Storyboard and follows a Shot voice override when present. Environment views use the same visual, inline-import pattern. Place Ana in a Shot, optionally add Shot-only overrides, choose an exact Environment view as its background, write action/dialogue and direct the camera without leaving the enhancer. Subject chips beside a Shot's Action insert `<Subject N>` at the cursor; hovering or focusing a chip previews its identity image, description and assigned voice. The node summary, read-only **LLM handoff** and physical outputs are compiled from that same v3 graph.
...
Use **Storyboard · Content** for visible beats, timed action/dialogue, cast, background, scale and transitions; **Storyboard · Stage** for cast positions, movement and eyelines; **Storyboard · Camera** for the selected Shot's route and named aim targets; **Library** for Subjects, Environments and Files; **Look** for creative direction and global cinematography; and **Review & Generate** for compilation, exact media outputs, diagnostics and queueing. Content, Stage and Camera edit the same Shot in Studio Project v3.
...
Prompt Studio performs immediate local checks before generation. Backend **Review** starts in an explicit **Not run** state and offers **Run Prompt Studio** for this node or **Queue full workflow** when downstream outputs should also execute. After a run it adds analysis for continuity, camera authority, prompt clarity, and safe repair actions. Review location chips select the related Shot and open its owning Storyboard mode, Library section, or Look, with an honest section-level fallback for output-only locations. Findings can be dismissed by stable fingerprint and restored through **Show dismissed** without changing the report or workflow. **Import & source tools** stays collapsed at the end of Review and contains compatibility transfer and source-repair controls; physical media files are never embedded. A validated package can replace the project atomically or append Generations and Shots with collision-safe IDs, rejecting conflicting shared definitions instead of guessing a merge.
