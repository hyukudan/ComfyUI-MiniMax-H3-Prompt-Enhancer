...
| **Reference Wiring** | Separate loaders and hand-maintained slot labels | **Native Prompt Studio** with previews, semantic drag/drop, safe imports, and aligned prompt + picture/video/audio outputs |
...
| 🎛️ [**Prompt Studio**](docs/prompt_studio.md) | One Studio Project v3 with Storyboard, reusable Subjects and Environments, a compact Files library, contextual Stage/Camera editing, exact H3 outputs, and Review & Generate. |
...
### 3. Building a visual reference prompt in Prompt Studio

1. Add **MiniMax H3 Prompt Enhancer** or **MiniMax H3 GGUF Prompt Enhancer**, then select **Open Studio**.
2. In **Library · Subjects**, create Ana and attach her identity image and default voice. Trim the voice waveform to the exact fragment you want H3 to receive and use **Preview selection** to hear that bounded output. In **Library · Environments**, create any reusable place and attach its background views. **Library · Files** is the compact import and preview shelf for raw images, video and audio.
3. In **Storyboard · Content**, create or select a Shot. Choose the Place and active background view directly in the background card; add Ana and any Objects from their visual **Add from Library** cards. Choose Ana's effective **Appearance in this Shot** (Base or any saved Look), then write the action and dialogue. The compact creation bar is only for new reusable resources, so current assignments are not duplicated in a second row of technical controls. Add **This Shot** image or voice overrides only when that Shot must differ from Ana's reusable defaults. Required identity, Look and Place-view files are assigned to numbered H3 outputs automatically.
4. Use **Storyboard · Stage** for blocking and eyelines—the staging board uses identity portraits when available—then **Storyboard · Camera** for that same Shot's Start, Move and End direction. The opaque sticky **Editing** context keeps the selected Shot and horizontal Shot strip visible.
5. Open **Review & Generate**, compile Studio Project v3, inspect the derived `<Picture N>`, `<Video N>` and `<Audio N>` outputs, then queue this node.
6. Connect `enhanced_prompt` and each active numbered output directly to the identically named native H3 input: `ref_image_1 → ref_image_1`, `ref_video_1 → ref_video_1`, `ref_video_audio_1 → ref_video_audio_1`, or `ref_audio_1 → ref_audio_1`. Numbering starts at 1. The older `pictures`, `videos`, and `audios` list outputs remain for adapters and saved workflows.
...
graph LR
    subgraph Input
        A[Basic Prompt]
        B[Studio Project v3<br/>Storyboard · Library · Look]
...
1. **`MiniMaxH3PromptEnhancer`**: Remote API / OpenAI-compatible endpoint prompt enhancer with a native visual Studio Project, self-repair, resolution outputs, and aligned `pictures`, `videos`, `audios`, and `reference_project` outputs.
2. **`MiniMaxH3GGUFPromptEnhancer`**: Direct local GGUF prompt enhancer with the same native Studio Project and media outputs plus automatic process lifecycle management.
...
5. **`MiniMaxH3VisualReferenceDirector`**: Legacy compatibility node for workflows created before visual reference direction became native to Prompt Studio.
...
Prompt Studio opens on **Storyboard**, the native visual prompt-authoring surface where each selected Shot follows the working order **Shot setup → Action & dialogue → Shot references → Direction**. There is no separate Scene entity: a Shot is one continuous span between cuts, and its setup owns the active Place view, cast, Objects and references. A single sticky Shot strip holds selection, scope and Content/Stage/Camera navigation without repeating a second Shot inspector. **Dialogue & sound** gives each exact line independent Speaker, Channel (on-screen or V.O.), Delivery and Tone controls, and shows only the effective default or Shot-override voice. Structured Shot dialogue is promoted into the exact source contract sent to the LLM, including its stable `(Sx)` speaker owner, so it is neither rejected as invented nor detached from its selected voice. Files open contextually from a reference destination instead of occupying the start of the workflow. One authoritative **Studio Project v3** owns Files, Subjects, Places, Props, Generations, Shots, links and Look inside the enhancer; no second node or user-facing companion document is required.

The enhancer node stays compact through a primary **Open Studio** action and destination chips. Once Storyboard contains Shots, the old outer **Video description** editor is replaced by a read-only **What Prompt Studio sends** summary. **Edit Storyboard** opens the only authoring surface. The legacy description remains editable only while Studio has no Shots. Prompt Studio now defaults to a wide visual workspace (up to 1100 px and 92% of the viewport), migrates the former narrow 720 px preference automatically, becomes full-width below 700 px, and remains manually resizable. Every deliberate editor change is saved immediately into the authoritative v3 workflow data—there is no separate Save button.
...
For reference workflows, **Studio Project v3 is the source of truth**. Storyboard presents a selectable Shot strip and a visual Shot setup with the active Place view, portrait cards for cast, Object cards, action, dialogue, compact Shot references and direction. Create Ana in **Library · Subjects**: her master card shows the primary portrait, image/voice readiness and saved appearances. Expand Ana in a Shot to select the effective Base or alternate Look; a Look picture is shown on the card and becomes a deterministic numbered H3 image automatically. **Choose from Files** opens a contextual picker; unrelated Files never masquerade as assigned identity media. The integrated voice editor provides waveform trim and bounded preview, and Storyboard displays the one effective voice after resolving defaults and overrides. Selecting a Place automatically selects its first available view and exposes the active view explicitly, preventing an apparently configured but inactive background. Subject, Place and Object lists use their primary pictures, and Stage reuses Subject portraits instead of abstract tokens. User-facing surfaces show names and previews; `<Subject N>`, `<Picture N>` and `<Audio N>` remain internal and are revealed only by the advanced H3 mapping in Review.

**Library · Props** defines reusable physical designs such as a named car, phone, weapon, product or piece of furniture. Give `Car Y` a stable visual description and attach one or more design pictures from the same inline gallery. Add it from **Shot setup** or use its visual mention chip beside the action. Prompt Studio presents it as an Object/Prop while its internal H3 mapping automatically activates pictures and allocates deterministic outputs. The user never sees or assigns media slot numbers manually.
...
```text
Prompt Studio / Prompt Enhancer → enhanced_prompt ────────────────────────────────┐
Prompt Studio / Prompt Enhancer → pictures / videos / audios ─────────────────────┼→ H3 generation node
Prompt Studio / Prompt Enhancer → reference_project → Inspector / adapter ────────┘
```

Use **Storyboard · Content** for visible beats, timed action/dialogue, cast, Props, background, scale and transitions; **Storyboard · Stage** for cast positions, movement and eyelines; **Storyboard · Camera** for the selected Shot's route and named aim targets; **Library** for Subjects, Places, Props and Files; **Look** for creative direction and global cinematography; and **Review & Generate** for compilation, exact media outputs, diagnostics and queueing. Content, Stage and Camera edit the same Shot in Studio Project v3.

Every Shot needs a visible **Action**. For a legacy workflow's first migration, a newly created Shot may copy the still-visible Video description as its editable starting point. As soon as a Shot exists, its Action and authored beats/dialogue replace that exterior text at runtime; each additional Shot requires its own Action. This one-way handoff avoids duplicate typing without leaving two prompt authorities.

Creating a Subject defines reusable **Project default** identity and voice Files; active use is explicit so an unused library character cannot leak into a prompt. Every active Subject is deterministically rebuilt at the start of Ref2VA output in `subject_definitions` from its Studio name, stable identity description, numbered identity Pictures and voice Audio; the first Shot mention repeats that stable description, so a detail such as “very long dreadlocks” cannot disappear merely because the LLM omitted the Subject block. Storyboard exposes separate **This Shot** identity and voice override destinations, so local direction never mutates the reusable Subject. Cast the Subject in Storyboard to include it in that Shot.

Studio Project v3 stores reusable Files and their semantic links alongside when Subjects and Environments appear, Staging, action/dialogue beats, and camera Start/Path/End. At generation time one Python compiler deterministically derives the legacy runtime contracts, reference context and physical slot map. H3 receives natural prose and stable names, never raw XYZ, percentage timing, numeric aim angles, or internal IDs.

**Library · Files** is the compact project-wide shelf for batch import and unassigned material, not a required detour. Import identity pictures and voices from a Subject, Environment views from the Environment, and Shot-only references from Storyboard; each direct import also becomes a reusable File. Removing a Shot connection does not delete that File. Slot labels are derived only during compilation, with real H3 limits of 9 pictures, 3 videos, 3 video soundtracks aligned to their video slots, and 3 independent audio inputs per Generation.

Prompt Studio performs immediate local checks before generation. Each Shot reports **Ready**, **Ready with suggestions**, or **Needs attention** instead of claiming readiness unconditionally. Review follows one hierarchy: **Validate**, **Generate prompt only**, then the primary **Generate & Queue** action; **Show H3 mapping** reveals the exact advanced media-to-output contract. After a run Review adds analysis for continuity, camera authority, prompt clarity and safe repair actions. Findings can be dismissed by stable fingerprint and restored without changing the workflow. **Import & source tools** stays collapsed at the end.
...
The existing `validation_report` includes a versioned `diagnosticReport` with stable codes, locations, blocking policy, repair eligibility, fingerprints, and bounded Prompt Coach advice. Existing output names and positions remain intact. After the five compatibility reference outputs, native numbered outputs are appended for all H3 autogrow slots: `ref_image_1..9`, `ref_video_1..3`, `ref_video_audio_1..3`, and `ref_audio_1..3`. ComfyUI receives UI diagnostics through an ephemeral payload rather than another output.
...
The selected named latitude is preserved end to end for both API and GGUF execution. It is used by the writer instructions and the validator; `conservative_grounded` is not silently promoted to `enhanced_production` by the legacy `enhance_description` compatibility flag.
