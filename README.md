...
For reference workflows, **MiniMax H3 Visual Reference Director** is the connected source of truth. Choose **Open Director** to enter its focused **Compose · Library · Wiring · Look** workspace. Compose presents a selectable, draggable scene strip, a central stage for cast/background/action, a visual reference tray, reference lanes, and a contextual scene inspector. Cuts can be duplicated with their complete camera/reference state, reordered by drag or accessible arrow controls, and removed through an explicit two-step confirmation. Create and place a named subject such as Ana atomically from `+ Subject`, or create an Environment inline and immediately attach its Background picture; existing library subjects and sets remain reusable. Each card exposes the exact `<Subject N>` identity used by the LLM, renders its assigned portrait in the avatar, and names its connected voice and performance media; the selected environment view is previewed in the set header. **Dialogue & sound** adds exact lines for visible subjects, chooses delivery, previews connected voices and cut audio, and keeps voice overrides, exact-dialogue recordings and soundtrack roles visibly separate. Ambience continues to derive from visible action and the Prompt Enhancer Audio policy, avoiding a second conflicting prompt field. Every cut also exposes a visual **Start · Move · End** camera card, plus a compact movement summary in the scene strip, and opens that cut's native camera editor directly. The scene inspector's read-only **LLM handoff** derives `<Subject N>`, `<Picture N>`, `<Video N>` and `<Audio N>` from the same generation bindings, flags missing physical files, and can copy a concise scene brief without creating a competing prompt source. Select or drag a tray card onto that subject's Image, Voice or Performance target, the scene Background, or its Camera, Audio and Continuity lanes; alternatively, use the adjacent `+` action to choose a physical file. The Director validates the complete logical assignment before uploading and then writes asset, H3 binding, semantic relationship, scene use and physical source atomically; a failed document write rolls back the project while leaving the content-addressed input safely reusable. Green checked targets show which relationships are already resolved; their `×` action detaches that destination without deleting the reusable Library file, and unused H3 wiring is pruned automatically. Its Details, Staging, and Camera modes still edit the same native shot-plan v2 data. Library remains the full import, preview and management surface for reusable Subjects and Environments. Use Wiring to audit the exact physical H3 slots and semantic readiness together. Subject identity, default voice, and environment views can be connected before any shot exists; they become reusable project defaults. A selected shot may still add an explicit voice, performance, camera, soundtrack, or continuity override. The Director compiles both the LLM context and the ordered tensors from the same assignments:
...
Creating a Subject defines a reusable identity and an optional **Default voice** audio asset; active use is explicit so an unused library character cannot leak into a prompt. Activating the subject activates both its identity dependencies and default voice, while a shot-scoped voice reference overrides only that scene. New subjects are included in the current generation by default. For an existing subject, enable **Subjects → Use in prompts → Always include in Generation …**, or cast it in **Shots → Who’s in it**. Review warns when a saved identity is not active anywhere and therefore will not reach the LLM.
