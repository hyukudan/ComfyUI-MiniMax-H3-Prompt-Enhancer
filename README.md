...
4. Use **Compose · Stage** for blocking and eyelines, then **Compose · Camera** for the same Shot's Start, Move, and End direction. A sticky **Editing** context keeps the editorial Shot name, generation, duration and Shot strip visible in every mode, so switching tools never changes scope silently.
...
2. **`MiniMaxH3GGUFPromptEnhancer`**: Direct local GGUF prompt enhancer with the same native Compose project and media outputs plus automatic process lifecycle management.
...
5. **`MiniMaxH3VisualReferenceDirector`**: Legacy compatibility node for workflows created before Compose became native to Prompt Studio.
...
For reference workflows, **Prompt Studio itself is the source of truth**. Compose presents a selectable Shot strip, central stage, cast/background/action controls, visual reference tray, semantic destinations and contextual inspector. Create Ana in **Cast & Places**, attach her project-default identity picture and voice, place her in a Shot, optionally add Shot-only image/voice overrides, choose an exact Environment view as its background, write action/dialogue and direct the camera without leaving the enhancer. The read-only **LLM handoff** derives `<Subject N>`, `<Picture N>`, `<Video N>` and `<Audio N>` from the same generation bindings that feed the physical outputs. Imports and multi-document edits are atomic; detaching a Shot destination preserves reusable Library media and Environment views.
...
Use **Compose · Build** for visible beats, timed action/dialogue, cast, background, scale and transitions; **Compose · Stage** for cast positions, movement and eyelines; **Compose · Camera** for the selected Shot's route and named aim targets; **Cast & Places** for reusable Subjects and Environments; **Media** for references and physical slots; **Look** for creative direction and global cinematography; and **Review** for diagnostics. Build, Stage, and Camera edit the same selected object in `shot_plan_json` v2 rather than duplicate data. In T2V, Media may remain empty; in reference modes the enhancer carries the physical media itself.
...
Creating a Subject defines reusable **Project default** identity and voice assets; active use is explicit so an unused library character cannot leak into a prompt. Compose exposes separate **This Shot** identity and voice override destinations, so local direction never mutates the reusable Subject. New Subjects are included in the current generation by default. For an existing Subject, enable **Cast & Places → Subjects → Use in prompts**, or cast it in **Compose**. Review warns when a saved identity is not active anywhere and therefore will not reach the LLM.
