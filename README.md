...
Prompt Studio opens on **Storyboard**, the native visual prompt-authoring surface where Subjects, identity images, voices, Environments/backgrounds, action, dialogue and camera are assembled per Shot. **Dialogue & sound** gives each exact line independent Speaker, Channel (on-screen or V.O.), Delivery and Voice color controls; structured Voice colors compile to the same pitch, timbre and pacing direction as the main prompt palette. One authoritative **Studio Project v3** owns Files, Subjects, Environments, Generations, Shots, links and Look inside the enhancer; no second node or user-facing companion document is required. Its integrated destinations are **Storyboard, Library, Look, and Review & Generate**. A sticky **Editing** context shows the Shot name, generation, duration and **This Shot** scope while **Content**, **Stage**, and **Camera** edit that same selected Shot.
