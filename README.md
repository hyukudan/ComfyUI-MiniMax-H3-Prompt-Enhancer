...
For reference workflows, **Prompt Studio itself is the source of truth**. Compose presents a selectable scene strip, central stage, cast/background/action controls, visual reference tray, semantic destinations and contextual inspector. Create Ana, attach her identity picture and default voice, place her in a shot, choose a background, write action/dialogue and direct the camera without leaving the enhancer. The read-only **LLM handoff** derives `<Subject N>`, `<Picture N>`, `<Video N>` and `<Audio N>` from the same generation bindings that feed the physical outputs. Imports and multi-document edits are atomic; detaching a destination preserves reusable Library media and prunes only wiring no longer used anywhere.
