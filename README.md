...
For reference workflows, **MiniMax H3 Visual Reference Director** is the connected source of truth. Choose **Open Director** to enter its focused **Compose · Library · Wiring · Look** workspace. Compose presents a selectable scene strip, a central stage for cast/background/action, reference lanes, and a contextual scene inspector; its Details, Staging, and Camera modes still edit the same native shot-plan v2 data. Library imports and previews pictures, clips, and audio and manages reusable Subjects and Environments. Connect each media card to a subject identity/voice, environment/background, or shot performance/camera role, then use Wiring to audit the exact physical H3 slots and semantic readiness together. Subject identity, default voice, and environment views can be connected before any shot exists; they become reusable project defaults. A selected shot may still add an explicit voice, performance, camera, soundtrack, or continuity override. The Director compiles both the LLM context and the ordered tensors from the same assignments:
