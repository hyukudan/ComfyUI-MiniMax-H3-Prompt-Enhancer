...
**Library · Files** is the compact project-wide shelf: import each picture, video or audio once, then connect it to a Subject, Environment or the active Shot from its meaningful visual destination. Removing a Shot connection does not delete the reusable File. Slot labels are derived only during compilation, with real H3 limits of 9 pictures, 3 videos and 3 audio inputs per Generation.
