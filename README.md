...
Use **Shots** for visible beats, timed action/dialogue spans, subject scale relationships, transitions, and a compact camera summary; **Subjects** for identity and appearances; **Environments** for stable places and temporary states; **Media** for the Reference library and per-generation physical slots; **Camera** for the selected shot's visual planner; **Look** for creative direction and global cinematography defaults; and **Review** for diagnostics. Look presents scene-wide **Mood (tone)** as a searchable, grouped control with a description and anti-invention boundary on every option; it is separate from line-level Delivery and Voice color beside the Basic prompt. Its Visual Language browser is organized as family → era/technique → variant and includes nine project-original, provenance-recorded comparison thumbnails for the newly expanded animation profiles; they illustrate vocabulary and are explicitly not H3 result guarantees. Its Advanced view also offers experimental animation cadence on ones, twos, or threes for compatible drawn and physical-animation styles; this requests pose exposure rhythm and never changes FPS or duration. Camera and Shots edit the same `shot_plan_json` v2 data; the split adds workspace, not schema. The Camera workspace uses a four-corner isometric floor plus Top and Front views, makes elevation visible, exposes path shape, overall pace, speed change, per-position framing/angle/aim, and includes a four-second playback and scrubber that follows each spatial position's real timing. In T2V, media can remain empty. In I2V, the image still travels from a Load Image node to H3. Ref2VA pairs each logical reference with a physically connected file and slot. Chained projects repeat that binding step per generation so slots can be reused without changing logical identity.
...
`media_manifest` v2 separates stable logical references from generation-specific `<Picture N>`, `<Video N>`, and `<Audio N>` bindings. **+ Add reference** registers reusable metadata in the Reference library; it does not upload or connect a file. The selected reference immediately offers a compatible action such as **Assign to Generation 1 · Picture 1**; the physical file must still be connected to that same input on the H3 generation node. The project also owns subject identity, appearance-state libraries, logical environments with bounded reference views, temporary environment states, activation, and generation initial state. `shot_plan_json` v2 owns when those resources appear or transition, plus separate opening/action fields, optional relative action/dialogue beats, and camera Start/Path/End. A spatial camera path can contain 2–6 timed, draggable positions in subject- or scene-relative space. Subject-relative paths anchor to one named project subject when a Shot has multiple subjects, so **behind** and **keep framed** never silently refer to the wrong person. Every position aims at that subject/object anchor by default, can face along the route, or can use a custom direction. The canvas draws a separate aim reticle: drag it to change pan/tilt without moving the camera position. Coordinates remain an editor detail: the LLM receives natural H3 camera language such as **behind `<Subject 1>`** and **keeping the camera aimed at `<Subject 1>`**, not raw XYZ values.
